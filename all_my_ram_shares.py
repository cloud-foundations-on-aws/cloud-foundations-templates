...
def find_all_ram_shares(fAllCredentials: list, fStatus: str = None, fType: str = None) -> list:
	"""
	...
	for x in range(WorkerThreads):
		worker = FindRAMShares(work_queue)
		worker.daemon = True
		worker.start()
...
if __name__ == '__main__':
	...
	Credentials = get_all_credentials(args.Profiles, args.Regions, args.SkipProfiles, args.SkipAccounts,  args.RootOnly, args.Accounts, args.AccessRoles)
	...
	print(f"Found {len(AllRAMShares)} RAM shares across {AccountNum} accounts across {RegionNum} regions")
	print()
